...
func halve_along_plane(plane_normal : Vector3, cutting_direction : Vector3, cutting_speed : float):
	if plane_normal.is_zero_approx():
		push_error("plane_normal can't be zero.")
		return

	spawn_halve(plane_normal, cutting_direction, cutting_speed)
	spawn_halve(-plane_normal, cutting_direction, cutting_speed)
	...
	if show_debug_plane:
		...
		debug_plane_instance.transform.basis = Basis.looking_at(plane_normal)
...
func spawn_halve(direction : Vector3, cutting_direction : Vector3, cutting_speed : float):
	...
	rigid_body.apply_central_force(-direction * 150)
